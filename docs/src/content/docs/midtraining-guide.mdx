...
- 📊 Dual evaluation (domain + general performance)
...
Curriculum learning means starting with easy examples and progressively increasing difficulty—just like humans learn.

### Why It Works

Imagine learning calculus before algebra. You'd struggle! Neural networks are the same:
- **Easy examples first** → Build foundational patterns
- **Progressive difficulty** → Avoid getting stuck in poor solutions
- **Research shows** → 10-15% improvement on hierarchical tasks
...
✅ Use for: Math problems, code complexity, scientific papers (natural difficulty ordering)

❌ Skip for: General web text, random documents (no clear difficulty hierarchy)
</Aside>
...
for epoch in range(10):
    ...
    if detector.is_forgetting(threshold=0.10):
        ...
        # Take action:
        # - Increase general data mixing (10% → 20%)
...
Don't just watch domain perplexity improve—you might be destroying general capability!
...
The dataset classes are fully implemented with:
- Proper structure and interfaces
- Educational documentation
- Domain-specific configurations

**Next Step: HuggingFace Integration 🚧**

To use actual datasets, integrate:
- `codeparrot/github-code-clean` for code
- `hendrycks/MATH` for math
- arXiv papers for science

See `src/transformer/domain_datasets.py` for placeholder locations.
</Aside>
...
Too low → Slow adaptation, wasted compute
...
```python
# Good: Track both!
print(f"Domain perplexity: {domain_ppl:.1f} (↓ is good)")
print(f"General perplexity: {general_ppl:.1f} (→ is good)")
...
```python
# Math: Clear difficulty levels → Use curriculum ✅
math_curriculum = create_math_curriculum(total_epochs=9, difficulty_levels=3)

# Code: Has complexity levels → Use curriculum ✅
code_curriculum = create_code_curriculum(total_epochs=9)

# General text: No clear hierarchy → Skip curriculum ❌
...
2. **Lower learning rate** by 50% (e.g., 1e-5 → 5e-6)
...
<div style="border: 1px solid var(--sl-color-gray-5); border-radius: 0.5rem; padding: 1rem;">
<h3 style="margin-top: 0;">🔬 Experiment</h3>
...
<div style="border: 1px solid var(--sl-color-gray-5); border-radius: 0.5rem; padding: 1rem;">
<h3 style="margin-top: 0;">🚀 Integrate Datasets</h3>
<p>Add HuggingFace datasets in <code>domain_datasets.py</code> for production use</p>
</div>

<div style="border: 1px solid var(--sl-color-gray-5); border-radius: 0.5rem; padding: 1rem;">
<h3 style="margin-top: 0;">💬 Share Results</h3>
